//
// VENDORED from three.js `dev`, examples/jsm/tsl/display/SSAONode.js
//
// Why a copy and not an import: this node does not exist in any published
// release. It is in dev only -- 0.185.1, the newest tag at the time of
// writing, 404s on this path -- and the project is on 0.183.1. Copying one
// self-contained file beats pinning the whole engine to an unreleased branch,
// and beats a third-party package that declares a peer range our three is not
// in and brings its own scene pass to fight ours.
//
// Why at all: GTAONode has no answer for depth discontinuities, which is what
// puts a halo around objects. This one does -- its own docs say the built-in
// blur "rejects samples across depth discontinuities" -- and it denoises
// itself, so it replaces both the AO pass and the separate DenoiseNode.
//
// LOCAL EDIT, and the only one: `getScreenPositionFromClip` was added to
// three's PostProcessingUtils after 0.183.1, so it is defined below instead of
// imported. It is copied verbatim from dev.
//
// When a release ships SSAONode, delete this file and import it normally.
//

import { RenderTarget, Vector2, TempNode, QuadMesh, NodeMaterial, RendererUtils, RedFormat } from 'three/webgpu';
import { reference, logarithmicDepthToViewZ, viewZToPerspectiveDepth, getViewPosition, vogelDiskSample, interleavedGradientNoise, nodeObject, Fn, float, NodeUpdateType, uv, uniform, Loop, vec2, vec4, int, dot, max, clamp, length, screenCoordinate, PI2, texture, passTexture, context } from 'three/tsl';
import { depthAwareBlur } from './depthAwareBlur.js';

/** Copied from three dev, src/nodes/utils/PostProcessingUtils.js. */
