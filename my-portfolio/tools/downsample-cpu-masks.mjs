 ...
 *
 * The Floor shader uses a big PNG mask (`grass-floor-1.png`) for **CPU-side**
 * triangle weighting (figuring out where to spawn grass blades). A KTX2
 * version exists for GPU sampling but can't be drawn onto a 2D canvas, so the
 * original PNG copy is loaded separately. It weighs ~880 kB which is a *lot*
 * for the critical loading path.
 *
